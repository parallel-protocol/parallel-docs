...
description: Stake PRL as sPRL1 or sPRL2 in the Parallel app, step by step. How monthly epochs, ParaBoost, fee rewards, the 28-day unstaking cooldown and early-exit penalty work.
...
| Unstaking | 28-day cooldown, or early exit with a penalty | 28-day cooldown, or early exit with a penalty |
...
An **epoch** is one calendar month in the PRL tokenomics, so it runs 28 to 31 days depending on the month. It sets two things: the window over which ParaBoost is computed, and the schedule on which fees are shared with stakers. The unstaking cooldown is not an epoch; it is a separate, fixed 28 days. See the [epoch concept](/governance/parallel-governance-token-prl/tokenomics/epoch-concept).
...
1. **Start the cooldown.** On the Stake page, click **Unstake**, enter the amount and confirm. This opens a fixed 28-day cooldown. During the cooldown the tokens stay locked and your ParaBoost is reset.
...
:::warning
An early exit right after you start the cooldown costs up to half of your stake. Unless you need the tokens now, let the cooldown run its 28 days.
:::
...
<FAQ items={[
  {
    ...
    answer: "sPRL1 if you want governance rights and fee rewards without exposure to wETH or impermanent loss. sPRL2 if you want the 2.5x ParaBoost weight and are comfortable holding an 80/20 PRL/wETH liquidity position. Both can be unstaked after a 28-day cooldown."
  },
  {
    question: "How long is an epoch, and is the unstaking cooldown the same thing?",
    answer: "An epoch is one calendar month, so 28 to 31 days depending on the month. Epochs set the ParaBoost calculation window and the fee distribution schedule. The unstaking cooldown is separate and fixed at 28 days."
  },
  {
  ...
  {
    question: "Can I unstake without waiting the 28 days?",
