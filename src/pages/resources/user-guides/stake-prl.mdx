...
- A wallet that supports the chain (MetaMask, Rabby, a Safe, or any WalletConnect wallet).
...
2. Open [app.parallel.best/stake](https://app.parallel.best/stake) and choose **Stake PRL/wETH LP** (sPRL2).
3. Enter the amount of BPT, approve it the first time, then click **Stake** and confirm.

**Let the app build the position**

The staking transactions accept either PRL or BPT for sPRL2 (the MCP `stake` tool builds "PRL/BPT into sPRL2"). When the app offers a PRL input on the sPRL2 tab, it joins the pool for you and stakes the resulting BPT in one flow. Check the quote: joining a pool with a single asset moves the pool price and can cost more than a balanced deposit.
...
To claim in the app, open the **Stake** page, find **Rewards** and click **Claim**. The [`claim_rewards`](/agents/mcp/tools) MCP tool builds the same transactions.
...
You can delegate your voting power without moving your tokens: on the Stake page, open **Your votes**, click **Change delegate**, and enter an address or choose **Delegate to self**. Delegation only affects Snapshot votes; you keep your sPRL, your ParaBoost and your rewards.
