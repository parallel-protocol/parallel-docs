...
Failures print `Error [CODE]: message` (JSON: `{ "success": false, "error": { "code", "message" } }`) and exit 1. Codes: `MISSING_OPTION`, `INVALID_ADDRESS`, `INVALID_AMOUNT`, `UNKNOWN_CHAIN`, `CHAIN_SUNSET`, `PARALLELIZER_NOT_DEPLOYED`, `COLLATERAL_NOT_SUPPORTED`, `INSUFFICIENT_BALANCE`, `WALLET_REQUIRED`, `INVALID_KEYSTORE`, `RPC_ERROR`, `UNKNOWN`.

Protocol-layer failures (caps, bridge limits, slippage, cooldowns) arrive as `code: "UNKNOWN"` with the reason prefixed in the message — e.g. `[EXCEEDS_BRIDGE_LIMIT] Daily bridge limit reached`. Match on the message prefix for these.
