...
36 tools. Reads return live data; tools marked **tx** return [unsigned transactions](/agents/mcp/transactions) for your wallet to sign.
...
| `deposit_savings` **tx** | Deposit USDp, receive sUSDp (`amount`, `chain`, `receiver`) |
| `withdraw_savings` **tx** | Redeem sUSDp back to USDp (`amount` in sUSDp, `chain`, `receiver`) |
...
| `mint` **tx** | Mint USDp from a collateral (`collateral`, `amount`, `chain`, `receiver`, `slippageTolerance?`) |
| `redeem` **tx** | Redeem USDp into one collateral |
| `redeem_proportional` **tx** | Redeem USDp into the whole collateral basket, market-neutral |
...
| `bridge` **tx** | Bridge USDp (`fromChain`, `toChain`, `amount`, `recipientAddress` — your own address, which sends and receives) |
...
| `stake` **tx** | Stake PRL into sPRL1, or PRL/BPT into sPRL2 |
| `unstake` **tx** | With `amount`: starts the cooldown. Without: withdraws matured requests |
| `claim_rewards` **tx** | Claim sPRL1/sPRL2 rewards — may return transactions on several chains |
...
| `prepare_x402_payment` **tx** | Builds a gasless x402 payment: returns typed data to sign, the facilitator settles on-chain |
| `prepare_convert_and_pay` **tx** | Redeem USDp to a collateral, then pay it — two steps, for recipients wanting a specific token |
