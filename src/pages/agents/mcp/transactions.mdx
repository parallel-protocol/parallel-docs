...
Impossible operations fail with a plain message instead of doomed calldata: a chain without a Parallelizer deployment, a collateral whose minting is paused, a bridge amount above the daily capacity (`EXCEEDS_BRIDGE_LIMIT`, with the remaining capacity and reset time).
