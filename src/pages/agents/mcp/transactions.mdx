...
Two tools go further and refuse to answer at all rather than mislead: `get_proof_of_solvency` (a ratio computed on incomplete data could declare the protocol insolvent when it is not) and any read whose partial answer would be structurally wrong. These fail with a `PARTIAL_READ` error naming the chains that could not be read.
