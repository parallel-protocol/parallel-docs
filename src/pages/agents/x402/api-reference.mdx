...
| `decimals` | `number` | No | Token decimals for string-price parsing. Default `18` (USDp). Use `6` for USDC-denominated prices. |
| `network` | `string` | Yes | Chain slug: `"ethereum"`, `"base"`, `"avalanche"`, or `"hyperevm"`. |
| `payTo` | `Address` | Yes | The merchant's receiving address. |
| `acceptedTokens` | `Address[]` | No | Token addresses the route accepts. Defaults to the supported stablecoins for the network. |
...
const client = new FacilitatorClient({ url: "https://facilitator.parallel.best" });
...
Both methods use a 10-second timeout. `verify` throws an [`X402RuntimeError`](/agents/x402/error-codes) on any failure; `settle` returns a result union on facilitator errors and throws only on connectivity issues.
